...
func Force_unregister(s *discordgo.Session, cmd *discordgo.InteractionCreate) {
	...
	if !exists {
		...
		*/
		...
		if !Execute {
			s.ChannelMessageSend(cmd.ChannelID, "The character who should get unregistered has admin permissions!")
			return
		}
		...
		s.ChannelMessageSend(cmd.ChannelID, "This player is now unregistered!")
