...
func Friends_ign_remove(s *discordgo.Session, cmd *discordgo.InteractionCreate) {
	//loading(s, cmd)
	...
	x := e1.FriendsIngamename

	e1.FriendsIngamename = []string{}

	for _, v := range x {
		if strings.ToLower(v) != strings.ToLower(IGN) {
			e1.FriendsIngamename = append(e1.FriendsIngamename, v)
		}
	}
